...
public class Utils {
    ...
    public static void printParts(Map<Integer, byte[]> map) {
        for (Map.Entry<Integer, byte[]> entry : map.entrySet()) {
            System.out.println(entry.getKey() + ":" + Base64.getEncoder().encodeToString(entry.getValue()));
        }
    }
    ...
    public static String parts2json(Map<Integer, byte[]> parts) {
        ...
        for (Map.Entry<Integer, byte[]> entry : parts.entrySet()) {
            tmp.put(entry.getKey(), Base64.getEncoder().encodeToString(entry.getValue()));
        }
        return gsonObject.toJson(tmp);
    }

    public static Map<Integer, byte[]> json2parts(String jsonString) {
        ...
        for (Map.Entry<Integer, String> entry : map.entrySet()) {
            ...
            byte[] val = Base64.getDecoder().decode(entry.getValue());
            res.put(i, val);
        }
        return res;
    }
